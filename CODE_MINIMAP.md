...
### `tests/cli/c_smoke.c` (~930 lines)
End-to-end FFI smoke test (M8 + iters 24/25 expansions). Exercises lifecycle (incl. NULL-safety), i64 + u64 set/get roundtrip + error cases, bit access (bit_at/bit_len with 0/1/0xFF/0x100/-0x100 sweeps including out-of-range and sign-ignore semantics), arithmetic (add/sub/mul/div/mod/divMod), powm + invMod (with no-inverse case), set_bytes → arith → bytes() round-trip, sign / cmp / is_zero predicates, error returns. Built as `c-smoke` binary; runs via `c-smoke-run` step. Wired into `./test` as the third group alongside Zig units and 12029 GMP cross-checks. **2026-06-01 coverage expansion:** now exercises ALL 85 exports (was 38) — added gcd/lcm, jacobi/legendre/kronecker, isqrt/isqrt_rem/iroot/is_perfect_square, factorial/binomial/fibonacci, is_probably_prime/next_prime, bitwise (and/or/xor/not/shl/shr), neg/abs, the fits_* matrix, an integer overflow/boundary matrix across the C ABI (INT64_MIN/MAX round-trip; set_u64 above i64max and get_u64(-1) and get_i64(UINT64_MAX) all REFUSE rather than wrap), popcount/scan0/scan1, set_str/to_string (incl. BUFFER_TOO_SMALL + required length), seeded RNG determinism + bounds, and the orphaned fp_* ops (rational_binary, cmp/eq, sub/mul, to_binary, round_to_scale, get_f64_exact/base/scale/mantissa).
